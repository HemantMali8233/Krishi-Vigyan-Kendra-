...
const DataEntryForm = () => {
  ...
  useEffect(() => {
    ...
    const loadRecord = async () => {
      ...
      if (!id) {
        // For new records, pre-fill the start date with the selected year from the module
  ...
  const update = (k) => (e) => {
    ...
    // Validation: Year must match the selected year from the module
    if (k === 'startDate' && val) {
      ...
      if (!isAdmin && lockedYears.has(inputYear)) {
        ...
        const selectedYear = location.state?.selectedYear;
        if (selectedYear && selectedYear !== 'all') {
          if (inputYear !== parseInt(selectedYear, 10)) {
            errorMsg = `Year must be ${selectedYear}. To change the year, change it in "Select Year" dropdown.`;
          }
        }
      }
    }
  ...
  const handleCancel = () => {
    navigate(cancelPath, {
      state: {
        restoreFilters: {
          ...restoreFilters,
          year: location.state?.selectedYear || restoreFilters.year
        }
      }
    });
  };
    ...
    if (shouldValidate('tab1')) {
      ...
      if (!form.startDate) e.startDate = 'Start date is mandatory';
        ...
        if (selectedYear && selectedYear !== 'all') {
          ...
          if (inputYear !== parseInt(selectedYear, 10)) {
            e.startDate = `Year must be ${selectedYear}. To change the year, change it in "Select Year" dropdown.`;
          } else {
            delete e.startDate;
          }
        } else {
  ...
  const save = async (ev) => {
    ...
    if (!isAdmin && record.year && lockedYears.has(Number(record.year))) {
      setStatusModal({
        ...
        message: `The year ${record.year} is locked by the admin. To perform this operation, please contact the administrator.`,
        type: 'error'
      });
      return;
    }
    ...
    if (!isAdmin && id && navRecord && navRecord.year && lockedYears.has(navRecord.year)) {
      setStatusModal({
        title: 'Action Blocked',
        message: `The original record for year ${navRecord.year} is locked. You cannot modify locked data.`,
        type: 'error'
      });
      return;
    }
    ...
    try {
      ...
      navigate(cancelPath, {
        state: {
          restoreFilters: {
            ...restoreFilters,
            year: location.state?.selectedYear || restoreFilters.year || (form.startDate ? new Date(form.startDate).getFullYear() : new Date().getFullYear())
          }
        }
      });
    } catch (error) {
  ...
  const cancelPath = returnToPath
    ? returnToPath
    : (disciplineCode ? `/dashboard/data-entry/${disciplineCode}` : '/dashboard/data-entry');
